...
const App = () => {
  const [authenticated, setAuthenticated] = useState(false); //  useState used for managing the state within the component.
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/user', { credentials: 'include' })
      ...
      .then((body) => {
        if (body) {
          setUser(JSON.parse(body)); // Parse and set user details.
  ...
  // Login Handler Redirects the user to an Okta login page for authentication.
  const login = () => {
  ...
  // Logout Handler : Retrieves the XSRF-TOKEN from cookies for CSRF protection.
  const logout = () => {
